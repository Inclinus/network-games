                ...
                case SDL: // Si l'évenement est de type SDL
                    if(strcmp(event->instructions, "LEAVE") == 0){ // Si l'instruction est LEAVE
                        ...
                        *choseGameRunning = SDL_FALSE; // Le menu n'est plus actif
                        ...
                        pthread_cancel(network_thread); // On ferme le NETWORK listener
                        pthread_cancel(sdl_thread); // On ferm le SDL listener
                        tictactoe(choseGameClientSocket); // On lance le jeu
                    } else if(strcmp(event->instructions, "CONNECT4") == 0){ // Si l'instruction est CONNECT4
                        send(*choseGameClientSocket, "NCONNECT4", 9, 0); // Envoie le choix du jeu au serveur
                        *choseGameRunning = SDL_FALSE; // Le menu n'est plus actif
                        pthread_cancel(network_thread);// On ferme le NETWORK listener
                        pthread_cancel(sdl_thread); // On ferm le SDL listener
                        connect4(choseGameClientSocket); // On lance le jeu
                    }
                    break;
