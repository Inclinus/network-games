...
void *networkListen() { // Ecoute des évenements NETWORK
    ...
    while (program_launched) {
        ...
        if (recv(*clientSocket, data, 8, 0) <= 0) { // En cas de connexion interrompu, se déconnecte
            sendEvent(disconnectEvent);
            break;
        } else {
            if (strcmp("WAITTURN", data) == 0) {
                NG_Event *enemyTurnEvent = malloc(sizeof(NG_Event)); // Créations de l'évènement enemyTurnEvent
                if(enemyTurnEvent==NULL){
                    SDL_ExitWithError("ERROR ALLOCATING ENEMYTURNEVENT");
                }
                enemyTurnEvent->type = NETWORK; // Type de l'évènement enemyTurnEvent
                enemyTurnEvent->instructions = malloc(sizeof(char)*10); // Allocation mémoire de l'instruction de enemyTurnEvent
                if(enemyTurnEvent->instructions==NULL){
                    SDL_ExitWithError("ERROR ALLOCATING ENEMYTURNEVENT INSTRUCTIONS");
                }
                enemyTurnEvent->instructions = "ENEMYTURN"; // Instruction de enemyTurnEvent
                sendEvent(enemyTurnEvent); // Envoie de l'évènment enmyTurnEvent au serveur
                int px;
                int py;
                recv(*clientSocket, &px, sizeof(px), 0); // Attente des coordonnées px
                recv(*clientSocket, &py, sizeof(py), 0); // Attente des coordonnées py
                NG_Event *enemyPosEvent = malloc(sizeof(NG_Event)); // Créations de l'évènement enemyPosEvent
                ...
                sprintf(enemyPosEvent->instructions, "%d-%d", px, py); // création de l'instruction de enemyPosEvent
                sendEvent(enemyPosEvent); // Envoie de l'évènement
            } else {
